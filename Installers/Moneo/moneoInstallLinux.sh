...
yes | \cp /cyclenfs/CycleCloudDemo/Installers/Moneo/moneo@.service /etc/systemd/system
yes | \cp /cyclenfs/CycleCloudDemo/Installers/Moneo/moneo_publisher.service /etc/systemd/system
# Start the service delayed by 5 minutes to allow the VM to fully boot up and to avoid that cluster init will kill the service by restarting the network
